...
class sbbst:
    ...
    def __init__(self, entries_list = None):
        ...
        self.sumsizes = []
        self.listInOrder = []
        if type(entries_list) == list:
            for key, val in entries_list:
                self.head = self.insertNode(self.head, key, val)
    ...
    def query(self, start, end):
        return self.queryNode(node, start, end)

    def queryNode(self, node, start, end):
        if not node:
            return []
    ...
    def insertNode(self, node, key, val):
        ...
        if not node:
            ...
            node.left = self.insertNode(node.left, key, val)
        else:
            node.right = self.insertNode(node.right, key, val)
    ...
    def inOrderNodes(self, node=-1):
        if node == -1:
            node = self.head
        if node:
            return self.inOrderNodes(node.left) + [node] + self.inOrderNodes(node.right)
        else:
            return []
    ...
    def getListInOrder(self, node=-1):
        ...
        if node:
            ...
            self.listInOrder.append(node.key)
